...
extern u8  time_flag_1ms;//在定时器中断中被累加，在IMU滤波是置零。
...
// section {0 1 2 3 4 5}
// set_speed { -4100  +4100}
static void square_wave_forawrd(char a_or_b, u8 section, s16 set_speed)
{
	s16 off_set = 0;
	if(a_or_b == 'a')
	{
...
void TIM1_UP_TIM10_IRQHandler(void)
{
	...
	{		
		if(1) //速度PI控制
		{
			...
			pid = -0.7*motor_a.set_speed - p - ki*i;
			
			pid = pid < -4000? 		-4000:pid;
			pid = pid >  4000?		 4000:pid;
			
			square_wave_forawrd('a', motor_a.current_section, pid);		
		}
		else
		{
			//square_wave_forawrd('a', motor_a.current_section, motor_a.set_speed);
			square_wave_forawrd('a', motor_a.current_section, 0);
		}		
	}
...
void TIM8_UP_TIM13_IRQHandler(void)
{
	...
	{		
		if(1)//速度PI控制
		{
			...
			pid = -0.7f*motor_b.set_speed - p - ki*i;
			
			pid = pid < -4000? 		-4000:pid;
			pid = pid >  4000?		 4000:pid;
			
			square_wave_forawrd('b', motor_b.current_section, pid);
		}
		else
		{			
			square_wave_forawrd('b', motor_b.current_section, motor_b.set_speed);	
		}		
	}
